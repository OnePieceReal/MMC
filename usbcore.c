...
extern __inline uint32_t USB_ReqGetStatus (void) {
  uint32_t n, m;
...
extern __inline uint32_t USB_ReqSetClrFeature (uint32_t sc) {
  uint32_t n, m;
...
extern __inline uint32_t USB_ReqGetDescriptor (void) {
  uint8_t  *pD;
  uint32_t len, n;
...
extern __inline uint32_t USB_ReqSetConfiguration (void) {
...
extern __inline uint32_t USB_ReqSetInterface (void) {
  USB_COMMON_DESCRIPTOR *pD;
